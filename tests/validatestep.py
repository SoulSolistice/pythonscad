...
def check_hole_nesting(entities, problems):
    ...
    for fid, outer, inner, outer_pts, drop, key in faces:
        for hid in inner:
            ...
            if not hpts:
                continue
            # The hole's first vertex, not a point interior to it. Probing
            # with an interior point finds the hole inside more faces, and the
            # intervening-loop test below then fires on concentric rings -
            # step-nested-rings - for geometry OpenCASCADE reads as one solid of
            # exactly the mesh's volume. The stricter probe was measured against
            # a kernel and it was the probe that was wrong, not the file.

...
- **`IFace2::Check` counts are not Import Diagnostics faulty faces.** 82 and 87
  ...
  are structural. On the band family the dialog counts *higher* than the API, 32
  against 3 and 5 against 2.
- **Fault codes seen** (`swFaultEntityErrorCode_e`): 7 `swEdgeVertexNotLie`,
  13 `swEdgeVerticesTouch` — an edge whose two vertices coincide, which is how a
  periodic face's seam is written, though a code 13 is *not* by itself evidence
  that a seam of ours caused it; see *Refuted claims* — 17 `swFaceBadEdge`, 21,
  24 `swFaceFaceInconsistency`, 30 `swTopolNotG1Continuous`.
...
multiples of 24."** Numerology. The refusal rule explains the split mechanically
and predicts it; the alignment story predicts nothing.
...
96 and is clean, so there it is had for nothing. **What the taper costs is
sharper than "it makes the exporter refuse facets": it is a fault source in
SOLIDWORKS in its own right.** Asking the exporter where its refusals fall
settles the mechanism — at `$fn` 96, by tenth of the sweep, refused 60 12 0 0 0 0
0 0 0 55 against claimed 149 96 97 97 97 97 98 96 96 149. All refusals are in the
two ramps where `f = max(0, min(1, t/0.2, (1-t)/0.2))` changes shape, and not one
is in the constant middle. The refusal rule fires on **the profile changing
shape**, where a cubic through the stations cannot follow a piecewise-linear
taper through its corners — not on the helix, the tessellation, or the wall.
